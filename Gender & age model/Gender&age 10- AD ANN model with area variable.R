...
MLR_train_data <- read.csv("C:/Users/dss/Desktop/Paper/Prediction model Data/Gender&age 10- AD MLR train data.csv")
test_data <- read.csv("C:/Users/dss/Desktop/Paper/Prediction model Data/Gender&age 10- AD test data.csv")
...
MLR_train_data <- MLR_train_data[,c(2,15:47,5:14)]
...
test_data <- test_data[,c(2,15:47,5:14)]

Area_code <- c("11", "26", "27", "28", "29", "30", "31", "41", "42", "43", "44", "45", "46", "47", "48", "49")
names(MLR_train_data) <- c("DT", paste0("Area", Area_code), paste0("A",seq(from=10, to=80, by=5),rep=""), "M", "F",
                           "x1", "x2", "x3", "x4", "x5", "z1", "z2", "z3", "z4", "y")
names(test_data) <- c("DT", paste0("Area", Area_code), paste0("A",seq(from=10, to=80, by=5),rep=""), "M", "F",
                      "x1", "x2", "x3", "x4", "x5", "z1", "z2", "z3", "z4", "y")
...
for (i in 1:200){
  {
    ...
    nn<-neuralnet(y~., data=train_data[,2:44], linear.output=T, stepmax=1e+10, hidden=c(42,42), threshold=0.001)
    #plot(nn,rep="best")
  }
  {
    predict<-compute(nn,train_data[,2:43])
  ...
  {
    predict<-compute(nn,val_data[,2:43])
  ...
  {
    predict<-compute(nn,test_data[,2:43])
...
result <- result[result$validr2 > 0.886,]
result <- result[result$testr2 > 0.886,]
result

setwd("C:/Users/dss/Desktop/Paper/Prediction model Data")
write.csv(train_data, file="Gender&age 10- AD ANN train data.csv", row.names = FALSE)
write.csv(val_data, file="Gender&age 10- AD ANN valid data.csv", row.names = FALSE)
write.csv(test_data, file="Gender&age 10- AD ANN test data.csv", row.names = FALSE)

all_data <- read.csv("C:/Users/dss/Desktop/Paper/Atopic Data/Gender&age 10- AD data.csv")
...
all_data <- all_data[,c(2,15:47,5:14)]

Area_code <- c("11", "26", "27", "28", "29", "30", "31", "41", "42", "43", "44", "45", "46", "47", "48", "49")
names(all_data) <- c("DT", paste0("Area", Area_code), paste0("A",seq(from=10, to=80, by=5),rep=""), "M", "F",
                     "x1", "x2", "x3", "x4", "x5", "z1", "z2", "z3", "z4", "y")

prediction <- compute(nn, all_data[,2:43])
prediction <- matrix(prediction$net.result)
prediction[prediction<0] <- 0
write.csv(prediction, "Gender&age 10- AD ANN with area variable prediction data.csv", row.names = FALSE)
write.csv(all_data, "Gender&age 10- AD ANN all data.csv", row.names = FALSE)
