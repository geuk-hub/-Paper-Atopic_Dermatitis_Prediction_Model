...
data <- data[,c(1,49:50,42:43,48)]

column <- c("DT", "MW0.5", "MW4", "x5", "z1", "y")
names(data) <- column
...
nn<-neuralnet(y~., data=train_data[,2:6], linear.output=T , hidden=c(2,2), threshold=0.0000005)
plot(nn,rep="best")
}
{
predict<-compute(nn,train_data[,2:5])
...
{
predict<-compute(nn,val_data[,2:5])
...
{
predict<-compute(nn,test_data[,2:5])
...
write.csv(train_data, file="nationwide ANN model train data.csv", row.names = FALSE)
write.csv(val_data, file="nationwide ANN model valid data.csv", row.names = FALSE)
write.csv(test_data, file="nationwide ANN model test data.csv", row.names = FALSE)

prediction <- compute(nn, data[,2:5])
prediction <- matrix(prediction$net.result)
write.csv(prediction, "nationwide ANN model prediction data.csv", row.names = FALSE)
