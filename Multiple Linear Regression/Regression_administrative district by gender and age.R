...
re <- lm(y~M+A+x5+z1+M:A+A:x5+A:z1, data)
summary(re)
vif(re)
anova(re)
...
model <- lm(y~M+A+x5+z1+M:A+A:x5+A:z1, train_data)
summary(model)
vif(model)
anova(model)
...
write.csv(train_data, file="Administrative district model by gender and age MLR train data.csv", row.names = FALSE)
write.csv(test_data, file="Administrative district model by gender and age MLR test data.csv", row.names = FALSE)

prediction <- matrix(predict(model, data))
write.csv(prediction, "Administrative district model by gender and age MLR prediction data.csv", row.names = FALSE)
